 ...
 - 사용형식(일반 조인)
   ...
   (사용예)2020년 1월 재품별 매입집계를 조회하시오.
    ...
    SELECT A.BUY_PROD AS 제품코드, B.PROD_NAME AS 제품명,
     ...
     WHERE A.BUY_PROD = B.PROD_ID 
           AND A.BUY_DATE BETWEEN TO_DATE('20200101') AND TO_DATE('20200131')--조인조건
  ...
  (사용예)상품테이블에서 판매가가 50만원이상인 제품을 조회하시오.
    ...
    SELECT A.PROD_ID AS 상품코드, A.PROD_NAME AS 상품명, B.LPROD_NM AS 분류명, C.BUYER_NAME AS 거래처명,
     ...
     WHERE A.PROD_LGU = B.LPROD_GU AND A.PROD_BUYER = C.BUYER_ID AND A.PROD_PRICE>=500000
     ORDER BY 5 DESC;
    ...
    SELECT A.PROD_ID AS 상품코드, A.PROD_NAME AS 상품명, B.LPROD_NM AS 분류명, C.BUYER_NAME AS 거래처명,
           A.PROD_PRICE AS 판매가격
      FROM PROD A
     INNER JOIN LPROD B ON(A.PROD_LGU = B.LPROD_GU AND A.PROD_PRICE>=500000)
